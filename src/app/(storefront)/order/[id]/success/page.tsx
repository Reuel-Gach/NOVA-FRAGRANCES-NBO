...
export default async function OrderSuccessPage({
  ...
  const whatsappMsg = encodeURIComponent(
    `Hi Banice, I have just placed order #${orderId.slice(0, 8).toUpperCase()} on Nova Fragrances for Ksh ${Number(order.total_price).toLocaleString()}. Please confirm dispatch!`
  );
  ...
  return (
    <main className="min-h-screen bg-[#060908] text-white p-4 md:p-8 flex items-center justify-center selection:bg-emerald-500 selection:text-black">
      <div className="max-w-xl w-full mx-auto">
        ...
        <div className="space-y-3">
          ...
          <button 
            ...
            className="w-full bg-[#121A16] hover:bg-[#1A2620] border border-emerald-500/30 text-emerald-400 font-extrabold uppercase tracking-widest py-3.5 rounded-xl transition-all flex items-center justify-center gap-2 text-xs shadow"
          >
      ...
      {/* Print-specific CSS styling to make printed receipt look pristine */}
      <style jsx global>{`
        @media print {
          body * {
            visibility: hidden;
